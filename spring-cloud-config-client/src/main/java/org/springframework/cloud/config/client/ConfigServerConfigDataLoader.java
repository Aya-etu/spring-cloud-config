...
public class ConfigServerConfigDataLoader implements ConfigDataLoader<ConfigServerConfigDataResource>, Ordered {
		...
		try {
			...
			for (String label : labels) {
				...
				if (result != null) {
					...
					ConfigData configData = createConfigDataManyOptions(propertySources, resource);
					if (configData != null) {
						return configData;
					}
				}
			}
			throw new ConfigClientFailFastException("None of labels " + Arrays.toString(labels) + " found", null);
		}
		catch (Exception e) {
	...
	private ConfigData makeConfigData(List<PropertySource<?>> propertySources,
	...
	private ConfigData createConfigDataManyOptions(List<PropertySource<?>> propertySources,
			ConfigServerConfigDataResource resource) {
	...
	private String generateException(Exception e) {
		if (e instanceof HttpServerErrorException httpException
				&& MediaType.APPLICATION_JSON.includes(httpException.getResponseHeaders().getContentType())) {
